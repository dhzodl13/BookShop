		...
		<c:otherwise>
			<c:forEach var="order" items="${orderList }">
				<div id="table_content">
					...
					<div class="table_content">${order.user_name }(${order.order_user_email })</div>
					...
					<div class="table_content">
						<input type="button" value="상세정보" onclick="location.href='OrderDetail.ad?order_num=${order.order_num }&order_status=${order.order_status }'">
					</div>
				</div>
			</c:forEach>
		</c:otherwise>
...
<div class="page"  style="text-align: center;">
		<c:choose>
			<c:when test="${ordPageInfo.pageNum > 1}">
				<input class="num" type="button" value="이전" onclick="pageMove(${ordPageInfo.pageNum-1})">
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${ordPageInfo.pageNum < ordPageInfo.maxPage}">
				<input class="num" type="button" value="다음" onclick="pageMove(${ordPageInfo.pageNum+1 })">
			</c:when>
			<c:otherwise>
