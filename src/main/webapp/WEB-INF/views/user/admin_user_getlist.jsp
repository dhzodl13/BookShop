	...
	<c:choose>
		...
		<c:choose>
			<c:when test="${userPageInfo.pageNum > 1}">
				<input class="num" type="button" value="이전" onclick="pageMove(${userPageInfo.pageNum - 1})">
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${userPageInfo.pageNum < userPageInfo.maxPage}">
				<input class="num" type="button" value="다음" onclick="pageMove(${userPageInfo.pageNum + 1})">
			</c:when>
			<c:otherwise>
