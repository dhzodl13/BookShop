...
<div class="page"  style="text-align: center;">
		<c:choose>
			<c:when test="${qnaPageInfo.pageNum > 1}">
				<input class="num" type="button" value="이전" onclick="pageMove(${qnaPageInfo.pageNum - 1})">
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${qnaPageInfo.pageNum < qnaPageInfo.maxPage}">
				<input class="num" type="button" value="다음" onclick="pageMove(${qnaPageInfo.pageNum + 1})">
			</c:when>
			<c:otherwise>
